...
@DisplayName("generate_token")
@ExtendWith(ConfigBeforeCallback.class)
public class AuthenticationTests extends AbstractCommonOperations implements ApiTestWithPayload {

	private static final Map<String, ApiTestResponseBase> TEST_DATA = new HashMap<>();

	private static final String API_URL = "/authentication/token/";
	private static final String RESOURCE_FOLDER = "/authentication/generatetokentest/";
	private static final String COMPLETE_RESOURCE_PATH = Constants.BASE_PACKAGE_PATH + RESOURCE_FOLDER;

	@Tag(TestTags.GENERATE_TOKEN)
	...
	public void enhanceRequest(RequestSpecification request, ApiTestRequestBase requestBase) {
		...
		//request.headers("rubric_header_ip_port", requestBase.getRubricHeaderIpPort());
	...
	@Nested
	@ExtendWith(ConfigBeforeCallback.class)
	class GenerateOtp extends AbstractCommonOperations implements ApiTestWithPayload {

		private static final String API_URL = "/authentication/otp/generate";
		private static final String RESOURCE_FOLDER = "/authentication/generateotptest/";
		private static final String COMPLETE_RESOURCE_PATH = Constants.BASE_PACKAGE_PATH + RESOURCE_FOLDER;

		@Tag(TestTags.GENERATE_OTP)
		@Tag(TestTags.POSITIVE_TEST)
		@ParameterizedTest(name = "Positive Test Case: [id: {0}, store_id: {1}, msisdn: {2}, http_status: {3}, request_payload_file_name: {4}, request_schema_file_name: {5}, response_schema_file_name: {6}, response_exact_matcher_file_name: {7}]")
		@CsvFileSource(resources = "/" + COMPLETE_RESOURCE_PATH
		+ "positive_test_cases.csv", numLinesToSkip = 1, nullValues = { "null" })
		public void generateOtpPositiveTests(
				@AggregateWith(PositiveTestRequestAggregator.class) PositiveTestRequest positiveTestRequest)
						throws IOException {
		...
		@CsvFileSource(resources = "/" + COMPLETE_RESOURCE_PATH
		+ "negative_test_cases.csv", numLinesToSkip = 1, nullValues = { "null" })
		public void generateOtpNegativeTests(
				@AggregateWith(NegativeTestRequestAggregator.class) NegativeTestRequest negativeTestRequest)
						throws IOException {
		...
		//Otp and token redis cache validation is pending!!
		@DisplayName("Otp Validate")
		@Nested
		@ExtendWith(ConfigBeforeCallback.class) class DeleteBookmarksTest extends
		AbstractCommonOperations implements ApiTestWithPayload {

			private static final String API_URL = "/authentication/otp/validate"; private
			static final String RESOURCE_FOLDER = "/authentication/validateotptest/";
			private static final String COMPLETE_RESOURCE_PATH= Constants.BASE_PACKAGE_PATH + RESOURCE_FOLDER;

			@Tag(TestTags.VALIDATE_OTP)
			@Tag(TestTags.POSITIVE_TEST)
			@ParameterizedTest(name = "Positive Test Case: [id: {0}, store_id: {1}, msisdn: {2}, http_status: {3}, request_payload_file_name: {4}, request_schema_file_name: {5}, response_schema_file_name: {6}, response_exact_matcher_file_name: {7}]")
			@CsvFileSource(resources = "/" + COMPLETE_RESOURCE_PATH
			+ "positive_test_cases.csv", numLinesToSkip = 1, nullValues = { "null" })
			public void validateOtpPositiveTests(
					@AggregateWith(PositiveTestRequestAggregator.class) PositiveTestRequest positiveTestRequest)
							throws IOException {
			...
			@CsvFileSource(resources = "/" + COMPLETE_RESOURCE_PATH
			+ "negative_test_cases.csv", numLinesToSkip = 1, nullValues = { "null" })
			public void validateOtpNegativeTests(
					@AggregateWith(NegativeTestRequestAggregator.class) NegativeTestRequest negativeTestRequest)
							throws IOException {
